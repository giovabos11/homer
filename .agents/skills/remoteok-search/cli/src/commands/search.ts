...
export interface SearchOpts {
  ...
  jobage: number
  page: number
  limit: number // page size (client-side; the feed is one document)
  format: "json" | "table" | "plain"
}

export function filterJobs(jobs: RemoteOKJob[], opts: SearchOpts): RemoteOKJob[] {
  let out = jobs
  if (opts.query) out = out.filter((j) => jobMatchesQuery(j, opts.query!))
  if (opts.location) {
    const l = opts.location.toLowerCase()
    out = out.filter((j) => (j.location ?? "").toLowerCase().includes(l))
  }
  if (opts.tags.length > 0) {
    const want = opts.tags.map((t) => t.toLowerCase())
    out = out.filter((j) => {
      const have = (j.tags ?? []).map((t) => String(t).toLowerCase())
...
export async function runSearch(opts: SearchOpts): Promise<number> {
  try {
    const { jobs, fromCache } = await loadFeed()
    ...
    if (opts.format === "table") {
      ...
      process.stdout.write(
        JSON.stringify(
          {
            meta: { count: pageRows.length, page: opts.page, total: rows.length, cached: fromCache },
            results: pageRows,
          },
          null,
          2,
