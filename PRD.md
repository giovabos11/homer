...
### Round: notes are not questions (2026-07-31)

- **Advisories vs screening answers** — the tailor used to write every drafter/reviewer flag into `applications.answers_json` as a `{ status: 'needs_user' }` marker keyed `FLAG: …`. Those flags are transparency notes ("posting wants MongoDB, the profile has none", "no independently verified information about the company"), not questions anyone can answer, so every application landed in Ready for review with 6 to 13 permanently unanswerable "questions": Approve stayed locked and `autoSubmitWhenResolved` could never fire even though every real question had resolved from the standing answers. Notes now have their own shape and column: `Advisory { kind: 'gap' | 'unverified' | 'compensation' | 'location' | 'other', text }[]` in `applications.advisories_json` (migration `0004`), exposed as `Application.advisories` and on `GET /api/applications/:id/artifacts`. `answers` holds real form questions only. Advisories never enter the gate, the approve check or the apply driver's field plan, and `PATCH /answers` refuses to write them. `unresolvedQuestions()` additionally ignores advisory-shaped keys, so a row that predates the repair can never block again. The defaults-table catch-all ("Skills, tools, or experience not in the profile") is a policy statement rather than a question and is dropped from resolution entirely. An idempotent boot repair (`migrateApplicationAdvisories`) moves existing `FLAG:` keys and catch-all rows out of `answers` — including rows still carrying the legacy `FLAGGED_FOR_USER` sentinel — preserving anything the user actually typed.
- **Review modal: questions first, notes below** — the Screening answers tab keeps the editable question list (unresolved highlighted, standing-answer checkbox), then a collapsed, muted "Notes from drafting (N)" section: read-only, grouped by kind with hairline section rules, and explicit that nothing there is waiting on the user. Approve is enabled as soon as the real questions are resolved.
- **Contextual salary answer** — when a posting publishes its own range and the standing salary answer is a stance rather than a figure, the answer becomes "Aligned with the posted range ($130,000-$165,000)" instead of a bare "Open". A figure the user set always wins, a predicted range is never quoted as the employer's word, and with no standing answer at all the question stays unanswered with the range offered as a one-click suggestion. `salaryMinAcceptable` remains non-binding: a posted range starting below it produces a `compensation` advisory and nothing else.
- **Application answers as dropdowns, and validation that never fails on casing** — the Profile modal renders the enum-ish standing answers as selects driven by `STANDING_ANSWER_OPTIONS` in `apps/shared/types.ts` (sponsorship, relocation, clearance, notice period, start date, and the four voluntary EEO questions), each with an "Other" escape hatch and a date-picker reveal for a specific start date; salary expectation stays free text with quick-fill chips and helper copy. Server-side, `PUT /api/standing-answers` normalizes instead of rejecting: "No", "no" and "NO" are all `no`, typed values are snapped onto their canonical option ignoring case and punctuation, and free text is preserved where a value is not enumerable. Only a genuinely different value ("maybe" for sponsorship) is still a 400. This fixes the reported error where typing "No" into the sponsorship field was refused.

### Round: approve once, submit once (2026-07-31)

- **Approving is idempotent, and it shows.** Every "Approve & submit" click used to enqueue a fresh `apply` task with no dedupe (one live application had stacked three while the queue was paused, each of which would have driven the employer's form again on resume). `TaskQueue.enqueueUnique(type, dedupeKey)` now keeps at most one live task per `(type, applicationId)` and every apply path uses it (approve endpoint, tailor auto-submit, retry); the endpoint returns `ApproveResult { taskId, taskState, alreadyQueued, queuePosition, queuePaused, application }` with the same `taskId` on every replay; the apply worker no-ops when `submittedAt` is already set; and boot + sweep repairs collapse pre-existing duplicates onto the oldest task (`done`, `Superseded by an earlier identical task #N`, never retried, never counted as failed). The kanban card replaces the Approve button with a state read off `approvedAt` + the live task ("Approved — queued to apply" / "Approved — queue paused" with an inline Resume queue / "Applying now" / "Needs you" / "Apply failed"), the approve toast names the queue position or says the queue is paused and carries a Resume queue action, the review dialog shows an unobtrusive paused notice next to Approve (which reads "Approve & queue" while paused), and the Home strip counts approved-but-unsubmitted applications. Boot also skips jobs whose title is a search query rather than a posting (a `search-queries.md` regex leaked into a portal query and was stored as a title), recording the reason on the row instead of deleting it.
- **Offers are a classification, and email matching stops guessing.** `EmailClass` gained `offer`: a formal offer (compensation plus a start date or a response deadline) moves the job + application to `offer` and celebrates, where it previously classified as `reply_accepted` and landed in Interview, so the Offer column could never fill from email; a stated `respondBy` becomes a `deadline` schedule event and the employer's own figures go verbatim into the summary. `hired` and declining stay user actions. Application matching was `LIKE '%company%'` + first row wins, so a rejection could close the wrong role at an employer with two open applications; it is now scored — posting URL, then normalized company, then job-title similarity, then live-status and recency preference — with `emails.match_basis` recording which signal won (migration `0005`). When the top two candidates are indistinguishable the email is stored unlinked with `match_candidates_json`, **no application is touched**, and the Inbox asks "Which application is this about?" with the candidates as one-click choices resolved by `PATCH /api/emails/:id`.

### Round: what is actually applyable (2026-07-31)

Diagnosed from a real end-to-end apply run, then from the shape of the live queue behind it.

- **Blocker diagnosis was inventing causes.** Application 12 (mintmcp, Ashby, fit 78) parked with *"A Google reCAPTCHA is blocking mintmcp — Software Engineer. Solve it manually…"*. There was no captcha and no posting: the stored id `b3334a8b-…` was stale (freehire.me had served a renumbered id; the company's live Ashby board carries `34d8220f-…`), the page was Ashby's client-rendered "Job not found" shell, and that shell's stylesheet contains `.grecaptcha-badge { visibility: hidden }` — which the old substring matcher `/grecaptcha/i` read as a captcha wall. The user was sent to solve nothing, on a job that no longer existed. `detectCaptcha` now requires a **genuine blocking widget** (vendor container class, `recaptcha/api2/anchor|bframe` challenge iframe, or an explicit `render()` call) **AND a form context**; reCAPTCHA v3 (`api.js` + `grecaptcha.execute`) is invisible scoring, not a wall, and no longer parks anything. Explicit interstitial wording ("verify you are human", Cloudflare's browser check) still counts on its own.
- **Pre-apply liveness verification, before any form interaction.** `checkPostingLiveness(url)` runs first in the apply worker and again inside the Playwright driver on the rendered DOM, so **"dead posting" always wins over every other diagnosis**. For Ashby / Greenhouse / Lever the company board API is authoritative (`posting-api/job-board/{slug}`, `boards-api.greenhouse.io/v1/boards/{token}/jobs`, `api.lever.co/v0/postings/{site}`) rather than scraping — those pages answer 200 with an empty JS shell whether the posting exists or not, which is exactly how the mintmcp failure hid. Off-board URLs fall back to HTTP status plus pure text heuristics (404/410, "job not found", "no longer accepting applications", "position closed", genuinely empty shells). Deliberately conservative: 401/403/429/5xx and any network failure are **inconclusive, never expired**, and a JS app shell is never condemned for having no server-rendered text.
- **Re-resolution before giving up.** A dead posting on a queryable board is re-matched by normalized title (Jaccard fallback, narrowed by location) against the board's current listing. One confident match → the job's `canonical_url` and `external_id` are rewritten in place, an advisory records *"posting id changed; re-resolved to the current listing"*, and the apply continues against the fresh URL. Multiple matches or none → the job is marked `expired` and the task fails **terminally** (`TerminalFailure` → `queue.failNow`, no backoff retry — re-reading the same 404 changes nothing) with the board's current openings listed in the failure detail so the user can see what else that company has open. URL→board derivation covers all three ATSs plus `greenhouse:<token>:<id>` external ids, which is the only place the board token survives for a careers page hosting an embedded board.
- **Apply channels: the pipeline stops pretending non-forms are forms.** New derived+persisted column `jobs.apply_channel` (migration `0006`), exposed as `Job.applyChannel`: `ats_form | aggregator_redirect | email | unknown`. Of the 15 approved applications in the live database, **only 4 were `ats_form`** — 8 were `whatjobs.com/pub_api__cpl__…` syndication redirects and 3 were Hacker News "Who is hiring" comment threads. Neither is an application form, and the pipeline would have burned an apply run on each.
  - `aggregator_redirect` — the redirect chain is followed first (bounded hops, HTTP redirects plus one meta-refresh). Resolving to an `ats_form` URL on a different host rewrites `canonical_url` and proceeds normally; dead-ending on the aggregator's own site (which is what every live whatjobs link does — they 301 to `whatjobs.com/`) marks the job `needs_manual` with an advisory naming the final URL.
  - `email` — the contact address is extracted from the stored posting text (scored: proximity to "apply/email/send your resume" beats a bare mention; no-reply and placeholder shapes are refused), and a ready-to-send application email is drafted **into the Outbox requiring approval**, reusing the existing approval-gated send path. Nothing is ever sent by the apply worker. The draft names the tailored resume/cover-letter paths to attach, and is created once per application, not once per retry. No address → `needs_manual` with the reason.
  - Classification happens at intake (`upsertJob`) and is recomputed when an ATS sighting replaces an aggregator sighting.
- **`expired` and `needs_manual` are JobStatus values.** A card that cannot be submitted leaves Ready for review for a new **Manual / expired** kanban column (accent `--serious`, sitting immediately right of Ready for review) instead of sitting there implying an imminent submission. `applyState` reads the application status ahead of the task state, so the card says "Posting no longer available" or "Apply by hand" rather than a retry-shaped "Apply failed".
- **Park reasons are explicit discriminators.** `ApplyBlocked` carries `ParkReason` (`dead_posting | captcha | login_wall | unmatched_field | low_confidence | driver_manual`), the worker merges it onto the task payload, and the needs-attention card renders it as a badge next to the prompt. `dead_posting` never parks at all — it expires the job. `low_confidence` (the agent mapper giving up) is split from `unmatched_field` (a question that needs the user), because they ask different things of the reader.
- **Channel badges.** Kanban cards and the JobDrawer show the channel. The expected case (`ats_form`) is a quiet icon-only outline badge — spelling out "ATS form" on 300 cards is noise — while `Aggregator` (amber), `Email` (terracotta) and `Unclassified` (grey) keep their word, so the exceptions are what the eye catches. The drawer adds a one-paragraph panel for non-form channels explaining what Homer will and will not do.
- **Idempotent boot backfill.** `backfillApplyChannels` classifies every existing job and adds a "cannot be auto-submitted" advisory to each approved-but-unsubmitted application whose channel is not `ats_form`. Classification is a pure function of the row and advisories merge by normalized text, so repeat runs are no-ops. Live result on this install: **318 jobs → 215 `ats_form`, 50 `aggregator_redirect`, 50 `email`, 3 `unknown`**; of the 15 approved applications, **11 are not auto-applyable** (8 aggregator + 3 email).
- **Tests stay hermetic.** `AppContext.httpFetch` is injectable (`FetchLike`); the test world defaults to an offline stub, so no test reaches the open internet and an unreachable posting is treated as inconclusive rather than expired. `stubFetch({url: response})` builds route maps for the redirect/board cases.
