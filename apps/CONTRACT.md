...
- `POST /api/applications/:id/approve` → `{ taskId }` — user approval at the submit gate (FR-9/D1). **409 `answers_unresolved`** while any REAL question is still a `{ status: 'needs_user' }` marker. Advisories never count (see below).
